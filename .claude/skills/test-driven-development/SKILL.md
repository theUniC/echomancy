...
- `src/echomancy/domainmodel/game/__tests__/helpers.ts` - Available test utilities
...
**Always resolve the stack before asserting:**
```typescript
...
- Using mocks when real implementations are available

**Project-specific:** Prefer real `Game` instances over mocks. Use `createStartedGame()` and test helpers.
